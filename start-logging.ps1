...
if ($self -eq $true) {
    $command += " | Select-String -pattern `"(TrickSaber|AndroidRuntime|SongDetails|CRASH)`""
} elseif ($custom -ne "") {
...
# Prollly not needed
# elseif ($all -eq $false) {
#     $command += " | Select-String -pattern `"(TrickSaber|QuestHook|modloader|AndroidRuntime|CRASH)`""
# }
